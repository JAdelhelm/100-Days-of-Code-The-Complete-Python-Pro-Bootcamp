...
class UmbrellaChecker():
    ...
    def set_parameters(self):
        self.parameters = {
            ...
            "lon": self.longitude,
            "appid": self.API_KEY,
            # Die nächsten 4 timestamps, also je 3 Stunden, 3*4 = 12h
            "cnt": 4,
        }
    ...
    def get_ids(self):
        ...
        temperature_per_forecast = self.kelvin_to_celsius(temperature_main_temp)
    ...
    def send_sms_alert(self, txt_to_sms, account_sid, auth_token, phone_number_from, phone_number_to):
        """
        Send SMS via Twilio: https://www.twilio.com/de-de
        """
        txt_to_sms = inspect.cleandoc(txt_to_sms)
        txt_to_sms_clean = textwrap.dedent(txt_to_sms)

        client = Client(account_sid, auth_token)

        message = client.messages \
                        .create(
                            body=txt_to_sms_clean,
                            from_=phone_number_from,
                            to=phone_number_to
                        )

    def send_mail_alert(self, your_email, to_addrs_mail, your_password, timezone):
        """
        Send Mails via PythonAnywhere: https://www.pythonanywhere.com/
        """

        os.environ["UTC"] = timezone

        if len(self.save_message) == 0:
            self.check_for_umbrella()

        self.save_message = inspect.cleandoc(self.save_message)
        txt_to_sms_clean = textwrap.dedent(self.save_message)

        ### Send E-Mail to yourself ###
        msg = EmailMessage()
        msg.set_content(txt_to_sms_clean)
...
if __name__ == "__main__":
    ...
    u_checker = UmbrellaChecker(latitude=78.941769, longitude=45, API_KEY="", forecasts="4")
    u_checker.check_for_umbrella()
    u_checker.send_mail_alert(your_email="", your_password="", 
                              to_addrs_mail="", timezone="Europe/Berlin")
